...
import com.obolum.sdk.utils.RsaUtils;
import okhttp3.*;
 ...
 * @author zhu.q
 */
public class DefaultObolumClient implements ObolumClient {
    ...
    private static DefaultObolumClient INSTANCE;


    private DefaultObolumClient(List<Interceptor> appInterceptors, List<Interceptor> networkInterceptors) {
    ...
    public synchronized static DefaultObolumClient getInstance() {
        return getInstance(null, null);
    }

    public synchronized static DefaultObolumClient getInstance(List<Interceptor> appInterceptors) {
        return getInstance(appInterceptors, null);
    }

    public synchronized static DefaultObolumClient getInstance(List<Interceptor> appInterceptors, List<Interceptor> networkInterceptors) {
        if (INSTANCE == null) {
            INSTANCE = new DefaultObolumClient(appInterceptors, networkInterceptors);
        }
        return INSTANCE;
    }
